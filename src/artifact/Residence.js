import BuildingType from "./BuildingType.js";
import Race from "./Race.js";

const Residence = {
   ELF_01: "elf01",
   ELF_02: "elf02",
   ELF_03: "elf03",
   ELF_04: "elf04",
   ELF_05: "elf05",
   ELF_06: "elf06",
   ELF_07: "elf07",
   ELF_08: "elf08",
   ELF_09: "elf09",
   ELF_10: "elf10",
   ELF_11: "elf11",
   ELF_12: "elf12",
   ELF_13: "elf13",
   ELF_14: "elf14",
   ELF_15: "elf15",
   ELF_16: "elf16",
   ELF_17: "elf17",
   ELF_18: "elf18",
   ELF_19: "elf19",
   ELF_20: "elf20",
   ELF_21: "elf21",
   ELF_22: "elf22",
   ELF_23: "elf23",
   ELF_24: "elf24",
   ELF_25: "elf25",
   ELF_26: "elf26",
   ELF_27: "elf27",
   ELF_28: "elf28",
   ELF_29: "elf29",
   HUMAN_01: "human01",
   HUMAN_02: "human02",
   HUMAN_03: "human03",
   HUMAN_04: "human04",
   HUMAN_05: "human05",
   HUMAN_06: "human06",
   HUMAN_07: "human07",
   HUMAN_08: "human08",
   HUMAN_09: "human09",
   HUMAN_10: "human10",
   HUMAN_11: "human11",
   HUMAN_12: "human12",
   HUMAN_13: "human13",
   HUMAN_14: "human14",
   HUMAN_15: "human15",
   HUMAN_16: "human16",
   HUMAN_17: "human17",
   HUMAN_18: "human18",
   HUMAN_19: "human19",
   HUMAN_20: "human20",
   HUMAN_21: "human21",
   HUMAN_22: "human22",
   HUMAN_23: "human23",
   HUMAN_24: "human24",
   HUMAN_25: "human25",
   HUMAN_26: "human26",
   HUMAN_27: "human27",
   HUMAN_28: "human28",
   HUMAN_29: "human29",

   properties:
   {
      "elf01":
      {
         name: "Residence 1 (Elf)",
         raceKey: Race.ELF,
         level: 1,
         width: 2,
         height: 2,
         population: 31,
         coin: 80,
         key: "elf01"
      },
      "elf02":
      {
         name: "Residence 2 (Elf)",
         raceKey: Race.ELF,
         level: 2,
         width: 2,
         height: 2,
         culture: -3,
         population: 4,
         coin: 118,
         key: "elf02"
      },
      "elf03":
      {
         name: "Residence 3 (Elf)",
         raceKey: Race.ELF,
         level: 3,
         width: 2,
         height: 2,
         culture: -4,
         population: 5,
         coin: 148,
         key: "elf03"
      },
      "elf04":
      {
         name: "Residence 4 (Elf)",
         raceKey: Race.ELF,
         level: 4,
         width: 2,
         height: 3,
         culture: -12,
         population: 28,
         coin: 261,
         key: "elf04"
      },
      "elf05":
      {
         name: "Residence 5 (Elf)",
         raceKey: Race.ELF,
         level: 5,
         width: 2,
         height: 3,
         culture: -11,
         population: 10,
         coin: 314,
         key: "elf05"
      },
      "elf06":
      {
         name: "Residence 6 (Elf)",
         raceKey: Race.ELF,
         level: 6,
         width: 2,
         height: 3,
         culture: -6,
         population: 10,
         coin: 373,
         key: "elf06"
      },
      "elf07":
      {
         name: "Residence 7 (Elf)",
         raceKey: Race.ELF,
         level: 7,
         width: 2,
         height: 3,
         culture: -4,
         population: 12,
         coin: 425,
         key: "elf07"
      },
      "elf08":
      {
         name: "Residence 8 (Elf)",
         raceKey: Race.ELF,
         level: 8,
         width: 3,
         height: 3,
         culture: -27,
         population: 70,
         coin: 717,
         key: "elf08"
      },
      "elf09":
      {
         name: "Residence 9 (Elf)",
         raceKey: Race.ELF,
         level: 9,
         width: 3,
         height: 3,
         culture: -7,
         population: 20,
         coin: 811,
         key: "elf09"
      },
      "elf10":
      {
         name: "Residence 10 (Elf)",
         raceKey: Race.ELF,
         level: 10,
         width: 3,
         height: 3,
         culture: -9,
         population: 30,
         coin: 905,
         key: "elf10"
      },
      "elf11":
      {
         name: "Residence 11 (Elf)",
         raceKey: Race.ELF,
         level: 11,
         width: 3,
         height: 3,
         culture: -7,
         population: 30,
         coin: 1000,
         key: "elf11"
      },
      "elf12":
      {
         name: "Residence 12 (Elf)",
         raceKey: Race.ELF,
         level: 12,
         width: 3,
         height: 3,
         culture: -6,
         population: 40,
         coin: 1142,
         key: "elf12"
      },
      "elf13":
      {
         name: "Residence 13 (Elf)",
         raceKey: Race.ELF,
         level: 13,
         width: 3,
         height: 3,
         culture: -7,
         population: 40,
         coin: 1272,
         key: "elf13"
      },
      "elf14":
      {
         name: "Residence 14 (Elf)",
         raceKey: Race.ELF,
         level: 14,
         width: 3,
         height: 3,
         culture: -8,
         population: 40,
         coin: 1391,
         key: "elf14"
      },
      "elf15":
      {
         name: "Residence 15 (Elf)",
         raceKey: Race.ELF,
         level: 15,
         width: 3,
         height: 3,
         culture: -7,
         population: 50,
         coin: 1500,
         key: "elf15"
      },
      "elf16":
      {
         name: "Residence 16 (Elf)",
         raceKey: Race.ELF,
         level: 16,
         width: 2,
         height: 4,
         culture: -37,
         population: 20,
         coin: 1680,
         key: "elf16"
      },
      "elf17":
      {
         name: "Residence 17 (Elf)",
         raceKey: Race.ELF,
         level: 17,
         width: 2,
         height: 4,
         culture: -17,
         population: 30,
         coin: 1769,
         key: "elf17"
      },
      "elf18":
      {
         name: "Residence 18 (Elf)",
         raceKey: Race.ELF,
         level: 18,
         width: 2,
         height: 4,
         culture: -18,
         population: 40,
         coin: 1859,
         key: "elf18"
      },
      "elf19":
      {
         name: "Residence 19 (Elf)",
         raceKey: Race.ELF,
         level: 19,
         width: 2,
         height: 4,
         culture: -20,
         population: 40,
         coin: 1957,
         key: "elf19"
      },
      "elf20":
      {
         name: "Residence 20 (Elf)",
         raceKey: Race.ELF,
         level: 20,
         width: 4,
         height: 3,
         culture: -137,
         population: 200,
         coin: 3082,
         key: "elf20"
      },
      "elf21":
      {
         name: "Residence 21 (Elf)",
         raceKey: Race.ELF,
         level: 21,
         width: 4,
         height: 3,
         culture: -35,
         population: 150,
         coin: 3246,
         key: "elf21"
      },
      "elf22":
      {
         name: "Residence 22 (Elf)",
         raceKey: Race.ELF,
         level: 22,
         width: 4,
         height: 3,
         culture: -46,
         population: 190,
         coin: 3425,
         key: "elf22"
      },
      "elf23":
      {
         name: "Residence 23 (Elf)",
         raceKey: Race.ELF,
         level: 23,
         width: 4,
         height: 3,
         culture: -47,
         population: 110,
         coin: 3612,
         key: "elf23"
      },
      "elf24":
      {
         name: "Residence 24 (Elf)",
         raceKey: Race.ELF,
         level: 24,
         width: 3,
         height: 5,
         culture: -181,
         population: 450,
         coin: 4763,
         key: "elf24"
      },
      "elf25":
      {
         name: "Residence 25 (Elf)",
         raceKey: Race.ELF,
         level: 25,
         width: 3,
         height: 5,
         culture: -68,
         population: 170,
         coin: 5023,
         key: "elf25"
      },
      "elf26":
      {
         name: "Residence 26 (Elf)",
         raceKey: Race.ELF,
         level: 26,
         width: 3,
         height: 5,
         culture: -76,
         population: 180,
         coin: 5302,
         key: "elf26"
      },
      "elf27":
      {
         name: "Residence 27 (Elf)",
         raceKey: Race.ELF,
         level: 27,
         width: 3,
         height: 5,
         culture: -83,
         population: 200,
         coin: 5611,
         key: "elf27"
      },
      "elf28":
      {
         name: "Residence 28 (Elf)",
         raceKey: Race.ELF,
         level: 28,
         width: 4,
         height: 4,
         culture: -158,
         population: 400,
         coin: 6324,
         key: "elf28"
      },
      "elf29":
      {
         name: "Residence 29 (Elf)",
         raceKey: Race.ELF,
         level: 29,
         width: 4,
         height: 4,
         culture: -108,
         population: 200,
         coin: 6631,
         key: "elf29"
      },
      "human01":
      {
         name: "Residence 1 (Human)",
         raceKey: Race.HUMAN,
         level: 1,
         width: 2,
         height: 2,
         population: 31,
         coin: 88,
         key: "human01"
      },
      "human02":
      {
         name: "Residence 2 (Human)",
         raceKey: Race.HUMAN,
         level: 2,
         width: 2,
         height: 2,
         culture: -3,
         population: 4,
         coin: 127,
         key: "human02"
      },
      "human03":
      {
         name: "Residence 3 (Human)",
         raceKey: Race.HUMAN,
         level: 3,
         width: 2,
         height: 2,
         culture: -4,
         population: 5,
         coin: 155,
         key: "human03"
      },
      "human04":
      {
         name: "Residence 4 (Human)",
         raceKey: Race.HUMAN,
         level: 4,
         width: 2,
         height: 2,
         culture: -6,
         population: 6,
         coin: 184,
         key: "human04"
      },
      "human05":
      {
         name: "Residence 5 (Human)",
         raceKey: Race.HUMAN,
         level: 5,
         width: 3,
         height: 2,
         culture: -19,
         population: 32,
         coin: 328,
         key: "human05"
      },
      "human06":
      {
         name: "Residence 6 (Human)",
         raceKey: Race.HUMAN,
         level: 6,
         width: 3,
         height: 2,
         culture: -5,
         population: 10,
         coin: 373,
         key: "human06"
      },
      "human07":
      {
         name: "Residence 7 (Human)",
         raceKey: Race.HUMAN,
         level: 7,
         width: 3,
         height: 2,
         culture: -4,
         population: 12,
         coin: 437,
         key: "human07"
      },
      "human08":
      {
         name: "Residence 8 (Human)",
         raceKey: Race.HUMAN,
         level: 8,
         width: 3,
         height: 2,
         culture: -5,
         population: 10,
         coin: 482,
         key: "human08"
      },
      "human09":
      {
         name: "Residence 9 (Human)",
         raceKey: Race.HUMAN,
         level: 9,
         width: 4,
         height: 3,
         culture: -57,
         population: 150,
         coin: 1088,
         key: "human09"
      },
      "human10":
      {
         name: "Residence 10 (Human)",
         raceKey: Race.HUMAN,
         level: 10,
         width: 4,
         height: 3,
         culture: -9,
         population: 40,
         coin: 1157,
         key: "human10"
      },
      "human11":
      {
         name: "Residence 11 (Human)",
         raceKey: Race.HUMAN,
         level: 11,
         width: 4,
         height: 3,
         culture: -9,
         population: 40,
         coin: 1300,
         key: "human11"
      },
      "human12":
      {
         name: "Residence 12 (Human)",
         raceKey: Race.HUMAN,
         level: 12,
         width: 4,
         height: 3,
         culture: -9,
         population: 40,
         coin: 1523,
         key: "human12"
      },
      "human13":
      {
         name: "Residence 13 (Human)",
         raceKey: Race.HUMAN,
         level: 13,
         width: 4,
         height: 3,
         culture: -9,
         population: 50,
         coin: 1636,
         key: "human13"
      },
      "human14":
      {
         name: "Residence 14 (Human)",
         raceKey: Race.HUMAN,
         level: 14,
         width: 4,
         height: 3,
         culture: -10,
         population: 60,
         coin: 1826,
         key: "human14"
      },
      "human15":
      {
         name: "Residence 15 (Human)",
         raceKey: Race.HUMAN,
         level: 15,
         width: 4,
         height: 3,
         culture: -9,
         population: 70,
         coin: 2083,
         key: "human15"
      },
      "human16":
      {
         name: "Residence 16 (Human)",
         raceKey: Race.HUMAN,
         level: 16,
         width: 5,
         height: 2,
         culture: -38,
         population: 20,
         coin: 2160,
         key: "human16"
      },
      "human17":
      {
         name: "Residence 17 (Human)",
         raceKey: Race.HUMAN,
         level: 17,
         width: 5,
         height: 2,
         culture: -21,
         population: 30,
         coin: 2230,
         key: "human17"
      },
      "human18":
      {
         name: "Residence 18 (Human)",
         raceKey: Race.HUMAN,
         level: 18,
         width: 5,
         height: 2,
         culture: -23,
         population: 30,
         coin: 2340,
         key: "human18"
      },
      "human19":
      {
         name: "Residence 19 (Human)",
         raceKey: Race.HUMAN,
         level: 19,
         width: 5,
         height: 2,
         culture: -25,
         population: 40,
         coin: 2457,
         key: "human19"
      },
      "human20":
      {
         name: "Residence 20 (Human)",
         raceKey: Race.HUMAN,
         level: 20,
         width: 4,
         height: 3,
         culture: -85,
         population: 70,
         coin: 3103,
         key: "human20"
      },
      "human21":
      {
         name: "Residence 21 (Human)",
         raceKey: Race.HUMAN,
         level: 21,
         width: 4,
         height: 3,
         culture: -34,
         population: 150,
         coin: 3266,
         key: "human21"
      },
      "human22":
      {
         name: "Residence 22 (Human)",
         raceKey: Race.HUMAN,
         level: 22,
         width: 4,
         height: 3,
         culture: -44,
         population: 190,
         coin: 3445,
         key: "human22"
      },
      "human23":
      {
         name: "Residence 23 (Human)",
         raceKey: Race.HUMAN,
         level: 23,
         width: 4,
         height: 3,
         culture: -47,
         population: 110,
         coin: 3631,
         key: "human23"
      },
      "human24":
      {
         name: "Residence 24 (Human)",
         raceKey: Race.HUMAN,
         level: 24,
         width: 3,
         height: 5,
         culture: -181,
         population: 450,
         coin: 4763,
         key: "human24"
      },
      "human25":
      {
         name: "Residence 25 (Human)",
         raceKey: Race.HUMAN,
         level: 25,
         width: 3,
         height: 5,
         culture: -68,
         population: 170,
         coin: 5023,
         key: "human25"
      },
      "human26":
      {
         name: "Residence 26 (Human)",
         raceKey: Race.HUMAN,
         level: 26,
         width: 3,
         height: 5,
         culture: -76,
         population: 180,
         coin: 5302,
         key: "human26"
      },
      "human27":
      {
         name: "Residence 27 (Human)",
         raceKey: Race.HUMAN,
         level: 27,
         width: 3,
         height: 5,
         culture: -83,
         population: 200,
         coin: 5611,
         key: "human27"
      },
      "human28":
      {
         name: "Residence 28 (Human)",
         raceKey: Race.HUMAN,
         level: 28,
         width: 4,
         height: 4,
         culture: -158,
         population: 400,
         coin: 6324,
         key: "human28"
      },
      "human29":
      {
         name: "Residence 29 (Human)",
         raceKey: Race.HUMAN,
         level: 29,
         width: 4,
         height: 4,
         culture: -108,
         population: 200,
         coin: 6631,
         key: "human29"
      },
   },
};

Residence.keys = function()
{
   return Object.keys(Residence.properties);
};

Residence.values = function()
{
   return Object.values(Residence.properties);
};

Residence.keys().forEach(function(buildingKey)
{
   const building = Residence.properties[buildingKey];
   building.typeKey = BuildingType.RESIDENCE;
   building.race = Race.properties[building.raceKey];
   building.type = BuildingType.properties[building.typeKey];
});

if (Object.freeze)
{
   Object.freeze(Residence);
}

export default Residence;