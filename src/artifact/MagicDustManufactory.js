import BuildingType from "./BuildingType.js";
import Race from "./Race.js";

const MagicDustManufactory = {
   ELF_01: "elf01",
   ELF_02: "elf02",
   ELF_03: "elf03",
   ELF_04: "elf04",
   ELF_05: "elf05",
   ELF_06: "elf06",
   ELF_07: "elf07",
   ELF_08: "elf08",
   ELF_09: "elf09",
   ELF_10: "elf10",
   ELF_11: "elf11",
   ELF_12: "elf12",
   ELF_13: "elf13",
   ELF_14: "elf14",
   ELF_15: "elf15",
   ELF_16: "elf16",
   ELF_17: "elf17",
   ELF_18: "elf18",
   ELF_19: "elf19",
   ELF_20: "elf20",
   ELF_21: "elf21",
   ELF_22: "elf22",
   ELF_23: "elf23",
   HUMAN_01: "human01",
   HUMAN_02: "human02",
   HUMAN_03: "human03",
   HUMAN_04: "human04",
   HUMAN_05: "human05",
   HUMAN_06: "human06",
   HUMAN_07: "human07",
   HUMAN_08: "human08",
   HUMAN_09: "human09",
   HUMAN_10: "human10",
   HUMAN_11: "human11",
   HUMAN_12: "human12",
   HUMAN_13: "human13",
   HUMAN_14: "human14",
   HUMAN_15: "human15",
   HUMAN_16: "human16",
   HUMAN_17: "human17",
   HUMAN_18: "human18",
   HUMAN_19: "human19",
   HUMAN_20: "human20",
   HUMAN_21: "human21",
   HUMAN_22: "human22",
   HUMAN_23: "human23",

   properties:
   {
      "elf01":
      {
         name: "Magic Dust Manufactory 1 (Elf)",
         raceKey: Race.ELF,
         level: 1,
         width: 4,
         height: 3,
         population: -192,
         culture: 0,
         tier3Product: 56,
         key: "elf01"
      },
      "elf02":
      {
         name: "Magic Dust Manufactory 2 (Elf)",
         raceKey: Race.ELF,
         level: 2,
         width: 4,
         height: 3,
         population: -21,
         culture: -21,
         tier3Product: 75,
         key: "elf02"
      },
      "elf03":
      {
         name: "Magic Dust Manufactory 3 (Elf)",
         raceKey: Race.ELF,
         level: 3,
         width: 4,
         height: 3,
         population: -24,
         culture: -20,
         tier3Product: 91,
         key: "elf03"
      },
      "elf04":
      {
         name: "Magic Dust Manufactory 4 (Elf)",
         raceKey: Race.ELF,
         level: 4,
         width: 4,
         height: 4,
         population: -114,
         culture: -49,
         tier3Product: 151,
         key: "elf04"
      },
      "elf05":
      {
         name: "Magic Dust Manufactory 5 (Elf)",
         raceKey: Race.ELF,
         level: 5,
         width: 4,
         height: 4,
         population: -39,
         culture: -39,
         tier3Product: 176,
         key: "elf05"
      },
      "elf06":
      {
         name: "Magic Dust Manufactory 6 (Elf)",
         raceKey: Race.ELF,
         level: 6,
         width: 4,
         height: 4,
         population: -44,
         culture: -14,
         tier3Product: 204,
         key: "elf06"
      },
      "elf07":
      {
         name: "Magic Dust Manufactory 7 (Elf)",
         raceKey: Race.ELF,
         level: 7,
         width: 4,
         height: 4,
         population: -62,
         culture: -16,
         tier3Product: 232,
         key: "elf07"
      },
      "elf08":
      {
         name: "Magic Dust Manufactory 8 (Elf)",
         raceKey: Race.ELF,
         level: 8,
         width: 5,
         height: 4,
         population: -215,
         culture: -62,
         tier3Product: 323,
         key: "elf08"
      },
      "elf09":
      {
         name: "Magic Dust Manufactory 9 (Elf)",
         raceKey: Race.ELF,
         level: 9,
         width: 5,
         height: 4,
         population: -94,
         culture: -25,
         tier3Product: 361,
         key: "elf09"
      },
      "elf10":
      {
         name: "Magic Dust Manufactory 10 (Elf)",
         raceKey: Race.ELF,
         level: 10,
         width: 5,
         height: 4,
         population: -120,
         culture: -27,
         tier3Product: 395,
         key: "elf10"
      },
      "elf11":
      {
         name: "Magic Dust Manufactory 11 (Elf)",
         raceKey: Race.ELF,
         level: 11,
         width: 5,
         height: 4,
         population: -143,
         culture: -31,
         tier3Product: 430,
         key: "elf11"
      },
      "elf12":
      {
         name: "Magic Dust Manufactory 12 (Elf)",
         raceKey: Race.ELF,
         level: 12,
         width: 5,
         height: 4,
         population: -190,
         culture: -33,
         tier3Product: 464,
         key: "elf12"
      },
      "elf13":
      {
         name: "Magic Dust Manufactory 13 (Elf)",
         raceKey: Race.ELF,
         level: 13,
         width: 5,
         height: 4,
         population: -195,
         culture: -38,
         tier3Product: 502,
         key: "elf13"
      },
      "elf14":
      {
         name: "Magic Dust Manufactory 14 (Elf)",
         raceKey: Race.ELF,
         level: 14,
         width: 5,
         height: 4,
         population: -198,
         culture: -42,
         tier3Product: 537,
         key: "elf14"
      },
      "elf15":
      {
         name: "Magic Dust Manufactory 15 (Elf)",
         raceKey: Race.ELF,
         level: 15,
         width: 5,
         height: 4,
         population: -246,
         culture: -46,
         tier3Product: 571,
         key: "elf15"
      },
      "elf16":
      {
         name: "Magic Dust Manufactory 16 (Elf)",
         raceKey: Race.ELF,
         level: 16,
         width: 5,
         height: 4,
         population: -271,
         culture: -77,
         tier3Product: 618,
         key: "elf16"
      },
      "elf17":
      {
         name: "Magic Dust Manufactory 17 (Elf)",
         raceKey: Race.ELF,
         level: 17,
         width: 5,
         height: 4,
         population: -333,
         culture: -97,
         tier3Product: 665,
         key: "elf17"
      },
      "elf18":
      {
         name: "Magic Dust Manufactory 18 (Elf)",
         raceKey: Race.ELF,
         level: 18,
         width: 5,
         height: 4,
         population: -384,
         culture: -114,
         tier3Product: 719,
         key: "elf18"
      },
      "elf19":
      {
         name: "Magic Dust Manufactory 19 (Elf)",
         raceKey: Race.ELF,
         level: 19,
         width: 5,
         height: 4,
         population: -443,
         culture: -135,
         tier3Product: 775,
         key: "elf19"
      },
      "elf20":
      {
         name: "Magic Dust Manufactory 20 (Elf)",
         raceKey: Race.ELF,
         level: 20,
         width: 4,
         height: 6,
         population: -1280,
         culture: -369,
         tier3Product: 1,
         key: "elf20"
      },
      "elf21":
      {
         name: "Magic Dust Manufactory 21 (Elf)",
         raceKey: Race.ELF,
         level: 21,
         width: 4,
         height: 6,
         population: -708,
         culture: -226,
         tier3Product: 1,
         key: "elf21"
      },
      "elf22":
      {
         name: "Magic Dust Manufactory 22 (Elf)",
         raceKey: Race.ELF,
         level: 22,
         width: 4,
         height: 6,
         population: -817,
         culture: -267,
         tier3Product: 1,
         key: "elf22"
      },
      "elf23":
      {
         name: "Magic Dust Manufactory 23 (Elf)",
         raceKey: Race.ELF,
         level: 23,
         width: 4,
         height: 6,
         population: -943,
         culture: -314,
         tier3Product: 1,
         key: "elf23"
      },
      "human01":
      {
         name: "Magic Dust Manufactory 1 (Human)",
         raceKey: Race.HUMAN,
         level: 1,
         width: 3,
         height: 2,
         population: -100,
         culture: 0,
         tier3Product: 28,
         key: "human01"
      },
      "human02":
      {
         name: "Magic Dust Manufactory 2 (Human)",
         raceKey: Race.HUMAN,
         level: 2,
         width: 3,
         height: 2,
         population: -11,
         culture: -9,
         tier3Product: 38,
         key: "human02"
      },
      "human03":
      {
         name: "Magic Dust Manufactory 3 (Human)",
         raceKey: Race.HUMAN,
         level: 3,
         width: 3,
         height: 2,
         population: -13,
         culture: -9,
         tier3Product: 47,
         key: "human03"
      },
      "human04":
      {
         name: "Magic Dust Manufactory 4 (Human)",
         raceKey: Race.HUMAN,
         level: 4,
         width: 4,
         height: 3,
         population: -151,
         culture: -43,
         tier3Product: 113,
         key: "human04"
      },
      "human05":
      {
         name: "Magic Dust Manufactory 5 (Human)",
         raceKey: Race.HUMAN,
         level: 5,
         width: 4,
         height: 3,
         population: -31,
         culture: -25,
         tier3Product: 132,
         key: "human05"
      },
      "human06":
      {
         name: "Magic Dust Manufactory 6 (Human)",
         raceKey: Race.HUMAN,
         level: 6,
         width: 4,
         height: 3,
         population: -41,
         culture: -10,
         tier3Product: 154,
         key: "human06"
      },
      "human07":
      {
         name: "Magic Dust Manufactory 7 (Human)",
         raceKey: Race.HUMAN,
         level: 7,
         width: 4,
         height: 3,
         population: -52,
         culture: -10,
         tier3Product: 173,
         key: "human07"
      },
      "human08":
      {
         name: "Magic Dust Manufactory 8 (Human)",
         raceKey: Race.HUMAN,
         level: 8,
         width: 5,
         height: 4,
         population: -344,
         culture: -91,
         tier3Product: 323,
         key: "human08"
      },
      "human09":
      {
         name: "Magic Dust Manufactory 9 (Human)",
         raceKey: Race.HUMAN,
         level: 9,
         width: 5,
         height: 4,
         population: -126,
         culture: -22,
         tier3Product: 361,
         key: "human09"
      },
      "human10":
      {
         name: "Magic Dust Manufactory 10 (Human)",
         raceKey: Race.HUMAN,
         level: 10,
         width: 5,
         height: 4,
         population: -154,
         culture: -24,
         tier3Product: 395,
         key: "human10"
      },
      "human11":
      {
         name: "Magic Dust Manufactory 11 (Human)",
         raceKey: Race.HUMAN,
         level: 11,
         width: 5,
         height: 4,
         population: -157,
         culture: -27,
         tier3Product: 430,
         key: "human11"
      },
      "human12":
      {
         name: "Magic Dust Manufactory 12 (Human)",
         raceKey: Race.HUMAN,
         level: 12,
         width: 5,
         height: 4,
         population: -163,
         culture: -30,
         tier3Product: 464,
         key: "human12"
      },
      "human13":
      {
         name: "Magic Dust Manufactory 13 (Human)",
         raceKey: Race.HUMAN,
         level: 13,
         width: 5,
         height: 4,
         population: -200,
         culture: -33,
         tier3Product: 502,
         key: "human13"
      },
      "human14":
      {
         name: "Magic Dust Manufactory 14 (Human)",
         raceKey: Race.HUMAN,
         level: 14,
         width: 5,
         height: 4,
         population: -241,
         culture: -37,
         tier3Product: 537,
         key: "human14"
      },
      "human15":
      {
         name: "Magic Dust Manufactory 15 (Human)",
         raceKey: Race.HUMAN,
         level: 15,
         width: 5,
         height: 4,
         population: -283,
         culture: -42,
         tier3Product: 571,
         key: "human15"
      },
      "human16":
      {
         name: "Magic Dust Manufactory 16 (Human)",
         raceKey: Race.HUMAN,
         level: 16,
         width: 5,
         height: 4,
         population: -101,
         culture: -128,
         tier3Product: 618,
         key: "human16"
      },
      "human17":
      {
         name: "Magic Dust Manufactory 17 (Human)",
         raceKey: Race.HUMAN,
         level: 17,
         width: 5,
         height: 4,
         population: -333,
         culture: -97,
         tier3Product: 665,
         key: "human17"
      },
      "human18":
      {
         name: "Magic Dust Manufactory 18 (Human)",
         raceKey: Race.HUMAN,
         level: 18,
         width: 5,
         height: 4,
         population: -384,
         culture: -114,
         tier3Product: 719,
         key: "human18"
      },
      "human19":
      {
         name: "Magic Dust Manufactory 19 (Human)",
         raceKey: Race.HUMAN,
         level: 19,
         width: 5,
         height: 4,
         population: -443,
         culture: -135,
         tier3Product: 775,
         key: "human19"
      },
      "human20":
      {
         name: "Magic Dust Manufactory 20 (Human)",
         raceKey: Race.HUMAN,
         level: 20,
         width: 4,
         height: 6,
         population: -1280,
         culture: -369,
         tier3Product: 1,
         key: "human20"
      },
      "human21":
      {
         name: "Magic Dust Manufactory 21 (Human)",
         raceKey: Race.HUMAN,
         level: 21,
         width: 4,
         height: 6,
         population: -708,
         culture: -226,
         tier3Product: 1,
         key: "human21"
      },
      "human22":
      {
         name: "Magic Dust Manufactory 22 (Human)",
         raceKey: Race.HUMAN,
         level: 22,
         width: 4,
         height: 6,
         population: -817,
         culture: -267,
         tier3Product: 1,
         key: "human22"
      },
      "human23":
      {
         name: "Magic Dust Manufactory 23 (Human)",
         raceKey: Race.HUMAN,
         level: 23,
         width: 4,
         height: 6,
         population: -943,
         culture: -314,
         tier3Product: 1,
         key: "human23"
      },
   },
};

MagicDustManufactory.keys = function()
{
   return Object.keys(MagicDustManufactory.properties);
};

MagicDustManufactory.values = function()
{
   return Object.values(MagicDustManufactory.properties);
};

// Supplementary data.
MagicDustManufactory.properties[MagicDustManufactory.ELF_08].coin = -1940 / 24;
MagicDustManufactory.properties[MagicDustManufactory.ELF_08].supplies = -194 / 24;

MagicDustManufactory.properties[MagicDustManufactory.HUMAN_10].coin = -3580 / 24;
MagicDustManufactory.properties[MagicDustManufactory.HUMAN_10].supplies = -358 / 24;

MagicDustManufactory.keys().forEach(function(buildingKey)
{
   const building = MagicDustManufactory.properties[buildingKey];
   building.typeKey = BuildingType.MAGIC_DUST_MANUFACTORY;
   building.race = Race.properties[building.raceKey];
   building.type = BuildingType.properties[building.typeKey];

   building.coin = valueOrZero(building.coin);
   building.culture = valueOrZero(building.culture);
   building.population = valueOrZero(building.population);
   building.supplies = valueOrZero(building.supplies);
   building.tier1Product = valueOrZero(building.tier1Product);
   building.tier2Product = valueOrZero(building.tier2Product);
   building.tier3Product = valueOrZero(building.tier3Product);
});

function valueOrZero(value)
{
   return (value !== undefined ? value : 0);
}

if (Object.freeze)
{
   Object.freeze(MagicDustManufactory);
}

export default MagicDustManufactory;